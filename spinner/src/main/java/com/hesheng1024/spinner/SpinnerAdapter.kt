...
internal class SpinnerAdapter<T> : BaseAdapter {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (convertView == null) {
            ...
            if (mTextSize != -1f) {
                Log.d(TAG, "setTextSize ${tvItem.textSize}")
                tvItem.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSize)
            }

            Log.d(TAG, "${tvItem.textSize} $mTextSize")
            if (mTextColor != -1) {
                tvItem.setTextColor(mTextColor)
            }
            tvItem.gravity = mGravity
            if (mPaddingRect.width() != 0 || mPaddingRect.height() != 0) {
                tvItem.setPadding(mPaddingRect.left, mPaddingRect.top, mPaddingRect.right, mPaddingRect.bottom)
            }
        } else {
